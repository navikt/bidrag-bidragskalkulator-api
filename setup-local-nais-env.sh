...
kubectl exec --tty deployment/bidrag-bidragskalkulator-api -- \
  /busybox sh -c "/busybox cat /proc/1/environ | /busybox tr '\0' '\n'" \
  | grep -E 'TOKEN_X_WELL_KNOWN_URL|TOKEN_X_CLIENT_ID|AZURE_APP_TENANT_ID|AZURE_APP_CLIENT_SECRET|AZURE_APP_CLIENT_ID|AZURE_APP_WELL_KNOWN_URL|AZURE_OPENID_CONFIG_TOKEN_ENDPOINT|BIDRAG_SJABLON_URL|BIDRAG_PERSON_URL|SCOPE|TOKEN_X|NAIS_APP_NAME|KODEVERK_URL|BIDRAG_GRUNNLAG_SCOPE|BIDRAG_GRUNNLAG_URL|SAFSELVBETJENING_URL|FOERSTESIDE_GENERATOR_URL|BIDRAG_DOKUMENT_PRODUKSJON_URL' \
  > "$SECRETS_FILE"
